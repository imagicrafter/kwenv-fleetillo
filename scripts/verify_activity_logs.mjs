...
async function main() {
    ...
    // DEBUG: Check if 'bookings' table exists
    const { data: tableCheck, error: tableError } = await supabase
        .from('bookings')
        .select('count(*)', { count: 'exact', head: true });

    if (tableError) {
        log(`DEBUG: 'bookings' table check FAILED: ${tableError.message}`);
    } else {
        log(`DEBUG: 'bookings' table check SUCCESS. Table exists.`);
    }

    // DEBUG: Check function source
    // const { data: funcDetails, error: funcError } = await supabase
    //    .rpc('get_function_source', { func_name: 'trigger_booking_activity' });

    // if (funcError) {
    //    log(`DEBUG: rpc failed (expected if not defined): ${funcError.message}`);
    // }

    // Instead of system table query which might fail permissions, let's just re-verify the logic
    // We can try to run the EXACT query the trigger runs to see if it finds the route.
    ...
    if (foundRoute) {
        ...
        // DEBUG: Run the trigger's query logic manually
        const { data: manualCheck, error: manualError } = await supabase
            .from('routes')
            .select('route_name, vehicles!inner(name)')
            .contains('stop_sequence', [targetBookingId])
            .is('deleted_at', null)
            .order('route_date', { ascending: false })
            .limit(1);

        if (manualError) {
            log(`DEBUG: Manual query failed: ${manualError.message}`);
        } else if (manualCheck && manualCheck.length > 0) {
            log(`DEBUG: Manual query SUCCESS. Found route: ${manualCheck[0].route_name}`);
        } else {
            log(`DEBUG: Manual query FAILED. Could not find route using same logic as trigger.`);
            // The trigger uses: NEW.id = ANY(r.stop_sequence)
            // PostgREST '.contains' uses the @> operator.
            // Let's try to verify if stop_sequence is indeed an array of UUIDs including this ID.
            log(`DEBUG: Route stop_sequence: ${JSON.stringify(foundRoute.stop_sequence)}`);
            log(`DEBUG: Target Booking ID: ${targetBookingId}`);
        }
    } else {
