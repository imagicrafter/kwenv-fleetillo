    ...
    for (const booking of bookings) {
        ...
        if (serviceIds.length > 0) {
            ...
            const { error: updateError } = await supabase
                ...
                .eq('id', booking.id);
            if (updateError) {
                console.error(`Error updating booking ${booking.booking_number} (${booking.id}):`, updateError);
                errorCount++;
            }
            else {
                console.log(`Updated booking ${booking.booking_number} (${booking.id}) - Source: ${source}, IDs: ${serviceIds.join(', ')}`);
                updatedCount++;
            }
        }
        else {
            console.log(`Skipping booking ${booking.booking_number} (${booking.id}) - No service data found in items or legacy ID.`);
            skippedCount++;
        }
    }
    console.log('-'.repeat(80));
...
//# sourceMappingURL=backfill-service-ids.js.map