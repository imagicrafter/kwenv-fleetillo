...
const VEHICLE_SPECS = [
    { make: 'Ford', model: 'F-450 Service Truck', year: 2022, fuelType: 'diesel', capacity: 1500 },
    { make: 'Freightliner', model: 'M2 106 Pumper', year: 2021, fuelType: 'diesel', capacity: 2500 },
    { make: 'Isuzu', model: 'NPR HD', year: 2023, fuelType: 'diesel', capacity: 1200 },
    { make: 'Peterbilt', model: '337 Vacuum Truck', year: 2020, fuelType: 'diesel', capacity: 3000 },
    { make: 'Kenworth', model: 'T270 Service', year: 2022, fuelType: 'diesel', capacity: 1800 },
    { make: 'Hino', model: '268A', year: 2021, fuelType: 'diesel', capacity: 1400 },
    { make: 'International', model: 'CV515', year: 2023, fuelType: 'diesel', capacity: 1600 },
    { make: 'Ram', model: '5500 Chassis', year: 2022, fuelType: 'diesel', capacity: 1100 },
];
function parseArgs() {
    const args = process.argv.slice(2);
    let size = 'medium';
    let clear = true; // Clear by default for idempotent behavior
    let locationsPerCustomer = null;
    let depotsPerCity = null;
    let maxCities = null;
    let validateRoutes = false;
    for (const arg of args) {
        if (arg.startsWith('--size=')) {
            const sizeArg = arg.replace('--size=', '');
...
function generateTexasAddress(city) {
...
function formatTimeForDb(hours, minutes) {
...
async function clearDemoData() {
    ...
    if (demoCustomers && demoCustomers.length > 0) {
        ...
        if (bookingByCustomerErr)
            console.warn('Error clearing bookings by customer:', bookingByCustomerErr.message);
    }
    ...
    if (demoLocations && demoLocations.length > 0) {
        ...
        if (bookingByLocationErr)
            console.warn('Error clearing bookings by location:', bookingByLocationErr.message);
    }
    ...
    if (demoVehicles && demoVehicles.length > 0) {
        ...
        if (vehicleLocationErr)
            console.warn('Error clearing vehicle_locations:', vehicleLocationErr.message);
    }
    ...
    if (vehicleErr)
        console.warn('Error clearing vehicles:', vehicleErr.message);
    // Clear drivers with demo tag
    const { error: driverErr } = await supabase.from('drivers').delete().contains('tags', [DEMO_TAG]);
    if (driverErr)
        console.warn('Error clearing drivers:', driverErr.message);
...
async function generateCustomers(count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const input = {
            ...
            companyName,
            email: faker_1.faker.internet.email({ provider: 'example.com' }),
            phone: faker_1.faker.phone.number({ style: 'national' }),
            mobilePhone: faker_1.faker.phone.number({ style: 'national' }),
...
const depotsPerCityCount = new Map();
async function generateLocations(customerIds, config) {
    ...
    const locationNameCount = new Map();
    ...
    const depotLocationsByCity = new Map();
    const homeLocationsByCity = new Map();
    const customerLocationsByCity = new Map(); // Track customer locations by city for booking distribution
    const citiesWithCustomerLocations = new Set(); // Track cities that need depots/vehicles
...
async function generateServices(count) {
...
async function generateVehicles(count, serviceIds, homeLocationsByCity, depotLocationsByCity) {
    ...
    const citiesWithVehicles = new Set();
    // Track vehicle count per city for booking distribution
    const vehiclesPerCity = new Map();
    // Track city assignments for vehicle_locations population
    const vehicleCityAssignments = new Map(); // vehicleId -> city
    ...
    for (let i = 0; i < actualVehicleCount; i++) {
        ...
        if (allHomeLocationIds.length > 0) {
            ...
            if (citiesNeedingVehicles.length > 0) {
                ...
                assignedCity = citiesWithHomes.find(city => homeLocationsByCity.get(city)?.includes(homeLocationId));
                if (assignedCity) {
                    citiesWithVehicles.add(assignedCity);
                }
            }
...
async function generateDrivers(count, vehicleIds) {
    console.log(`\nGenerating ${count} drivers and assigning to vehicles...`);
    const driverIds = [];
    for (let i = 0; i < count; i++) {
        const firstName = faker_1.faker.person.firstName();
        const lastName = faker_1.faker.person.lastName();
        const hireDate = faker_1.faker.date.past({ years: 5 });
        const input = {
            firstName,
            lastName,
            email: faker_1.faker.internet.email({ firstName, lastName, provider: 'fleetillo.com' }),
            phoneNumber: faker_1.faker.phone.number({ style: 'national' }),
            licenseNumber: `TX${faker_1.faker.string.numeric(8)}`,
            licenseExpiry: faker_1.faker.date.future({ years: 3 }),
            licenseClass: 'C',
            status: 'active',
            hireDate,
            emergencyContactName: faker_1.faker.person.fullName(),
...
async function generateBookings(count, _customerIds, // Not used directly - we use locationToCustomer map instead
locationsByCustomer, customerLocationsByCity, vehiclesPerCity, serviceIds) {
    ...
    const priorityOptions = ['low', 'normal', 'normal', 'normal', 'high', 'urgent'];
    ...
    const locationsUsedPerDayPerCity = new Map(); // Map<dateKey, Map<city, Set<locationId>>>
    // Get location to customer mapping for booking creation
    const locationToCustomer = new Map();
    ...
    for (let day = 0; day < numDays; day++) {
        ...
        if (!locationsUsedPerDayPerCity.has(dateKey)) {
            locationsUsedPerDayPerCity.set(dateKey, new Map());
        }
        ...
        for (const [city, vehicleCount] of vehiclesPerCity) {
            ...
            if (!dayUsedLocations.has(city)) {
                dayUsedLocations.set(city, new Set());
            }
...
async function generateRoutes(count, vehicleIds, bookingIds) {
