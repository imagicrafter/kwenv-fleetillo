...
exports.findNearestDepot = findNearestDepot;
exports.canVehicleServiceBooking = canVehicleServiceBooking;
exports.clusterBookingsByDepot = clusterBookingsByDepot;
exports.getDefaultClusteringConfig = getDefaultClusteringConfig;
exports.getClusteringConfigAsync = getClusteringConfigAsync;
exports.isClusteringEnabled = isClusteringEnabled;
exports.isClusteringEnabledAsync = isClusteringEnabledAsync;
const logger_1 = require("../utils/logger");
const config_1 = require("../config");
const settings_service_1 = require("./settings.service");
const logger = (0, logger_1.createContextLogger)('ClusteringService');
/**
 ...
 */
function findNearestDepot(coordinates, depots) {
    if (depots.length === 0) {
        return null;
    }
 ...
 */
function canVehicleServiceBooking(vehicle, booking) {
    ...
    return bookingServiceIds.some((serviceId) => vehicle.serviceTypes.includes(serviceId));
}
/**
 ...
 */
function clusterBookingsByDepot(bookings, vehicles, depotLocations, clusterConfig) {
    ...
    const depots = [];
    const vehiclesByHomeLocation = new Map();
    ...
    for (const { booking, coordinates, locationCity } of bookings) {
        ...
        const canService = nearest.depot.vehicles.some((v) => canVehicleServiceBooking(v, booking));
        if (!canService) {
            unassigned.push({
                booking,
    ...
    const assignedCount = Array.from(depotClusters.values()).reduce((sum, cluster) => sum + cluster.length, 0);
 ...
 */
function getDefaultClusteringConfig(maxStopsPerVehicle) {
    return {
        maxRadiusMiles: config_1.config.routePlanning.maxClusterRadiusMiles,
        maxStopsPerVehicle,
    };
}
/**
 ...
 */
async function getClusteringConfigAsync(maxStopsPerVehicle) {
    try {
...
function isClusteringEnabled() {
    return config_1.config.routePlanning.enableClusteringV2;
}
/**
 * Check if clustering is enabled (async version - checks env var AND database setting)
 */
async function isClusteringEnabledAsync() {
    // First check env var (master kill-switch)
    if (!config_1.config.routePlanning.enableClusteringV2) {
        return false;
    }
