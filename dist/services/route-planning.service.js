...
exports.RoutePlanningServiceError = RoutePlanningServiceError;
/**
 * Error codes for route planning service
 */
exports.RoutePlanningErrorCodes = {
...
function getBookingCoordinates(booking) {
    ...
    if (booking.locationLatitude !== undefined &&
    ...
    if (booking.serviceLatitude !== undefined &&
 ...
 */
function hasValidCoordinates(booking) {
 ...
 */
function estimateTravelTimeMinutes(lat1, lon1, lat2, lon2, avgSpeedKmph, trafficBufferMultiplier) {
 ...
 */
async function createTimeAwareAllocation(bookings, vehicles, locationMap, servicesMap // serviceId -> averageDurationMinutes
) {
    ...
    while (unassigned.length > 0 && assignedSomething) {
        ...
        for (const state of vehicleStates) {
            ...
            if (state.usedMinutes >= MAX_DAILY_MINUTES)
                continue;
            ...
            if (nearestIdx !== -1 && nearestCoords) {
                ...
                const travelTime = state.hasLocation
                    ? estimateTravelTimeMinutes(state.currentLat, state.currentLon, nearestCoords.latitude, nearestCoords.longitude, params.avgTravelSpeedKmph, params.trafficBufferMultiplier)
                    : 0;
 ...
 */
async function optimizeBatch(bookings, options) {
    if (bookings.length === 0) {
        return {
            success: false,
            error: new RoutePlanningServiceError('Cannot optimize empty batch', exports.RoutePlanningErrorCodes.INVALID_INPUT),
        };
    }
    // Determine origin
    let origin;
    ...
    if (!routeResult.success) {
        return {
            success: false,
            error: new RoutePlanningServiceError(`Route optimization failed: ${routeResult.error?.message}`, exports.RoutePlanningErrorCodes.OPTIMIZATION_FAILED, routeResult.error),
        };
    }
    const response = routeResult.data;
    if (!response.routes || response.routes.length === 0) {
        return {
            success: false,
            error: new RoutePlanningServiceError('No routes returned from optimization', exports.RoutePlanningErrorCodes.OPTIMIZATION_FAILED),
        };
    }
    ...
    let firstVisitedBooking;
    let lastVisitBooking;
 ...
 */
async function generateUniqueRouteCode(date) {
...
async function findCompatibleVehicleBookings(bookings, serviceIdFilter) {
    ...
    if (incompatibleBookings.length > 0) {
        warnings.push(`${incompatibleBookings.length} booking(s) have services not supported by any available vehicle`);
    }
    return {
        compatibleBookings,
 ...
 */
function chunkBookings(bookings, size) {
    const chunks = [];
 ...
 */
async function previewRoutePlan(input) {
    ...
    if (!bookingsResult.success) {
        return {
            success: false,
            error: new RoutePlanningServiceError(`Failed to fetch bookings: ${bookingsResult.error?.message}`, exports.RoutePlanningErrorCodes.FETCH_FAILED, bookingsResult.error),
        };
    }
    ...
    const enhancedAllocation = await Promise.all(allocations.map(async (alloc) => {
        ...
        const availableLocations = (vehicleLocsData || []).map((vl) => ({
            id: vl.locations.id,
        ...
        const finalLocations = availableLocations.length > 0
            ? availableLocations
            : baseLocationsData?.map((loc) => ({ ...loc, isPrimary: false })) || [];
        return {
    ...
    return {
        ...
        data: {
            ...
            warnings,
            availableBaseLocations: (baseLocationsData || []).map((loc) => ({
                id: loc.id,
                name: loc.name,
                city: loc.city,
                state: loc.state,
...
//# sourceMappingURL=route-planning.service.js.map