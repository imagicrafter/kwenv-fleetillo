...
exports.GoogleMapsErrorCodes = exports.GoogleMapsServiceError = void 0;
...
exports.GoogleMapsServiceError = GoogleMapsServiceError;
/**
 * Error codes for Google Maps service errors
 */
exports.GoogleMapsErrorCodes = {
 ...
 */
function mapApiStatusToError(status, errorMessage) {
    switch (status) {
        case 'ZERO_RESULTS':
            return new GoogleMapsServiceError('No results found for the provided address', exports.GoogleMapsErrorCodes.ZERO_RESULTS, { status });
        case 'OVER_DAILY_LIMIT':
        case 'OVER_QUERY_LIMIT':
            return new GoogleMapsServiceError('Google Maps API quota exceeded', exports.GoogleMapsErrorCodes.QUOTA_EXCEEDED, { status }, true // Retryable with delay
            );
        case 'REQUEST_DENIED':
            return new GoogleMapsServiceError(errorMessage || 'Google Maps API request was denied', exports.GoogleMapsErrorCodes.REQUEST_DENIED, { status, errorMessage });
        case 'INVALID_REQUEST':
            return new GoogleMapsServiceError(errorMessage || 'Invalid request to Google Maps API', exports.GoogleMapsErrorCodes.INVALID_INPUT, { status, errorMessage });
        case 'UNKNOWN_ERROR':
        default:
            return new GoogleMapsServiceError(errorMessage || 'An unknown error occurred with Google Maps API', exports.GoogleMapsErrorCodes.API_ERROR, { status, errorMessage }, true // Retryable
            );
    }
}
/**
...
function validateApiKey() {
    ...
    if (!apiKey || apiKey.trim().length === 0) {
        ...
        return {
            success: false,
            error: new GoogleMapsServiceError('Google Maps API key is not configured', exports.GoogleMapsErrorCodes.MISSING_API_KEY),
        };
    }
...
function validateCoordinates(coords) {
    if (typeof coords.latitude !== 'number' || typeof coords.longitude !== 'number') {
        return {
            success: false,
            error: new GoogleMapsServiceError('Coordinates must be numeric values', exports.GoogleMapsErrorCodes.INVALID_COORDINATES, { coordinates: coords }),
        };
    }
    if (coords.latitude < -90 || coords.latitude > 90) {
        return {
            success: false,
            error: new GoogleMapsServiceError('Latitude must be between -90 and 90', exports.GoogleMapsErrorCodes.INVALID_COORDINATES, { latitude: coords.latitude }),
        };
    }
    if (coords.longitude < -180 || coords.longitude > 180) {
        return {
            success: false,
            error: new GoogleMapsServiceError('Longitude must be between -180 and 180', exports.GoogleMapsErrorCodes.INVALID_COORDINATES, { longitude: coords.longitude }),
        };
    }
    return { success: true };
}
...
async function fetchWithRetry(url, options = {}) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
        try {
            ...
            if (lastError.name === 'AbortError') {
                ...
                lastError = new GoogleMapsServiceError('Request timed out', exports.GoogleMapsErrorCodes.TIMEOUT, { timeoutMs }, true);
            }
    ...
    return {
        ...
        error: lastError instanceof GoogleMapsServiceError
            ? lastError
            : new GoogleMapsServiceError(lastError?.message || 'Network error', exports.GoogleMapsErrorCodes.NETWORK_ERROR, lastError, true),
    };
}
/**
...
async function geocodeAddress(input) {
    ...
    if (!input.address || input.address.trim().length === 0) {
        return {
            success: false,
            error: new GoogleMapsServiceError('Address is required for geocoding', exports.GoogleMapsErrorCodes.INVALID_ADDRESS),
        };
    }
    // Build API URL
    ...
    if (!firstResult) {
        return {
            success: false,
            error: new GoogleMapsServiceError('No results found for the provided address', exports.GoogleMapsErrorCodes.ZERO_RESULTS),
        };
    }
...
async function reverseGeocode(input) {
    ...
    if (!firstResult) {
        return {
            success: false,
            error: new GoogleMapsServiceError('No results found for the provided coordinates', exports.GoogleMapsErrorCodes.ZERO_RESULTS),
        };
    }
    // Convert and return the first result
    const result = (0, googlemaps_js_1.rawToGeocodingResult)(firstResult);
...
async function validateAddress(input) {
    ...
    if (addressParts.length === 0) {
        return {
            success: false,
            error: new GoogleMapsServiceError('At least one address component is required', exports.GoogleMapsErrorCodes.INVALID_ADDRESS),
        };
    }
...
async function getPlaceAutocomplete(input) {
    ...
    if (!input.input || input.input.trim().length === 0) {
        return {
            success: false,
            error: new GoogleMapsServiceError('Input text is required for autocomplete', exports.GoogleMapsErrorCodes.INVALID_INPUT),
        };
    }
    // Build API URL
    const params = new URLSearchParams({
 ...
 */
async function getDistanceMatrix(input) {
    ...
    if (input.origins.length === 0) {
        return {
            success: false,
            error: new GoogleMapsServiceError('At least one origin is required', exports.GoogleMapsErrorCodes.INVALID_INPUT),
        };
    }
    if (input.destinations.length === 0) {
        return {
            success: false,
            error: new GoogleMapsServiceError('At least one destination is required', exports.GoogleMapsErrorCodes.INVALID_INPUT),
        };
    }
    // Format locations
