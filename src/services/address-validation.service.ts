 ...
 */
const logger = createContextLogger('AddressValidationService');

/**
 * Address Validation service error
 */
export class AddressValidationServiceError extends Error {
 ...
 */
export const AddressValidationErrorCodes = {
  // Input errors
 ...
 */
const DEFAULT_OPTIONS: AddressValidationOptions = {
...
function validateApiKey(): Result<string> {
  ...
  if (!apiKey || apiKey.trim().length === 0) {
    ...
    return {
      success: false,
      error: new AddressValidationServiceError(
        'Google Maps API key is not configured',
        AddressValidationErrorCodes.MISSING_API_KEY
      ),
    };
  }
 ...
 */
function addressInputToString(input: AddressInput): string {
 ...
 */
function validateAddressInput(input: AddressInput): Result<string> {
  ...
  if (!addressString || addressString.length === 0) {
    return {
      ...
      error: new AddressValidationServiceError(
        ...
        AddressValidationErrorCodes.EMPTY_ADDRESS,
        { input }
      ),
    };
  }
 ...
 */
function locationTypeToConfidence(locationType: GeocodingLocationType): {
 ...
 */
function createStandardizedAddress(result: GeocodingResult): StandardizedAddress {
  const structured = extractStructuredAddress(result);
 ...
 */
function analyzeValidationIssues(
  input: AddressInput,
  ...
  if (inputAddress && resultAddress) {
    ...
    const changedParts = inputParts.filter(
      part => !resultParts.some(rp => rp.includes(part) || part.includes(rp))
    );
 ...
 */
function createComponentValidation(
  input: AddressInput,
  standardized: StandardizedAddress
): ComponentValidation[] {
...
async function getSuggestions(
  ...
  try {
    ...
    return result.data.slice(0, options.maxSuggestions || 3).map((pred: PlacePrediction) => ({
 ...
 */
export async function validateAddressWithDetails(
  input: AddressInput,
  options: AddressValidationOptions = {}
): Promise<Result<AddressValidationResponse>> {
  ...
  if (!apiKeyResult.success) {
    return apiKeyResult as unknown as Result<AddressValidationResponse>;
  }
  ...
  if (!inputResult.success) {
    ...
    const emptyResponse: AddressValidationResponse = {
      status: 'INVALID',
      isValid: false,
  ...
  if (!geocodeResult.success) {
    ...
    const isNotFound = error instanceof GoogleMapsServiceError &&
      error.code === GoogleMapsErrorCodes.ZERO_RESULTS;

    const issues: AddressValidationIssue[] = [{
      severity: 'ERROR',
      code: isNotFound ? 'ADDRESS_NOT_FOUND' : 'GEOCODING_FAILED' as ValidationIssueCode,
      message: isNotFound
    ...
    const failedResponse: AddressValidationResponse = {
      status: 'INVALID',
      isValid: false,
      confidence: 'UNVERIFIED',
      confidenceScore: 0,
  ...
  const { level: confidenceLevel, score: confidenceScore } =
    locationTypeToConfidence(geocodedData.locationType);
  ...
  const status = determineValidationStatus(
    issues,
    confidenceScore,
    mergedOptions.minConfidenceScore || 50
  );
...
export async function standardizeAddress(
  input: StandardizeAddressInput
): Promise<Result<StandardizeAddressResponse>> {
...
export async function geocodeWithValidation(
  input: GeocodeWithValidationInput
): Promise<Result<GeocodeWithValidationResponse>> {
  logger.info('Geocoding address with validation');

  // Convert input to AddressInput if string
  const addressInput: AddressInput = typeof input.address === 'string'
...
): Promise<Result<BatchValidationResponse>> {
  const startTime = Date.now();
  ...
  for (let i = 0; i < request.addresses.length; i += CONCURRENCY_LIMIT) {
    ...
    const batchResults = await Promise.all(
      batch.map(async (address, batchIndex) => {
        ...
        const validationResponse: AddressValidationResponse = result.success && result.data
          ? result.data
          : {
              status: 'UNKNOWN',
              isValid: false,
              ...
              issues: [{
                severity: 'ERROR',
                code: 'GEOCODING_FAILED' as ValidationIssueCode,
 ...
 */
export async function getValidatedCoordinates(
  ...
  if (!result.success || !result.data?.success || !result.data.coordinates) {
    return {
      success: false,
      error: new AddressValidationServiceError(
        result.data?.error || 'Failed to get coordinates',
        AddressValidationErrorCodes.GEOCODING_FAILED
      ),
    };
  }
