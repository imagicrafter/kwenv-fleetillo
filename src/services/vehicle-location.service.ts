...
import type {
  VehicleLocation,
  VehicleLocationRow,
  SetVehicleLocationInput,
} from '../types/vehicle-location';
 ...
 */
export async function getVehiclePrimaryLocation(
  vehicleId: string
): Promise<Result<VehicleLocation | null>> {
  const supabase = getAdminSupabaseClient() || getSupabaseClient();
...
export async function setVehicleLocations(
  ...
  try {
    ...
    if (deleteError) {
      logger.error('Failed to delete existing vehicle locations', {
        error: deleteError,
        vehicleId,
      });
    ...
    const { data, error } = await supabase.from('vehicle_locations').insert(rows).select();
 ...
 */
export async function removeVehicleLocation(
  vehicleId: string,
  locationId: string
): Promise<Result<void>> {
  const supabase = getAdminSupabaseClient() || getSupabaseClient();
 ...
 */
export async function setVehiclePrimaryLocation(
  vehicleId: string,
  locationId: string
): Promise<Result<void>> {
  const supabase = getAdminSupabaseClient() || getSupabaseClient();
