...
import { TravelMode, RoutingPreference, PolylineQuality } from '../types/google-routes';
import type {
  ComputeRoutesInput,
...
import {
  clusterBookingsByDepot,
  isClusteringEnabledAsync,
  getClusteringConfigAsync,
  type BookingWithCoordinates,
...
export interface VehicleAllocation {
  vehicleId: string;
  bookingCount: number;
  bookingIds?: string[]; // Specific booking IDs to assign (from clustering)
...
async function createTimeAwareAllocation(
  ...
  let vehicleAllowedBookings: Map<string, Set<string>> | null = null;
  const clusteringUnassigned: Booking[] = [];

  if (await isClusteringEnabledAsync()) {
    ...
    const depotLocations = new Map<
      string,
      { name: string; city: string; coordinates: { latitude: number; longitude: number } }
    >();
    ...
    const bookingsWithCoords: BookingWithCoordinates[] = [];
    ...
    const clusterConfig = await getClusteringConfigAsync(
      Math.ceil(MAX_DAILY_MINUTES / 30) // Estimate max stops based on 30 min average
    );

    const clusterResult = clusterBookingsByDepot(
      bookingsWithCoords,
      vehicles,
      depotLocations,
      clusterConfig
    ...
    for (const [depotId, assignments] of clusterResult.depotClusters) {
      ...
      for (const vehicle of vehicles) {
        if (vehicle.homeLocationId === depotId) {
          const allowedIds = vehicleAllowedBookings.get(vehicle.id) || new Set<string>();
    ...
    for (const unassigned of clusterResult.unassigned) {
      ...
      warnings.push(
        `Booking ${unassigned.booking.bookingNumber || unassigned.booking.id}: ${unassigned.reason}` +
  ...
  const getServiceDuration = (b: Booking) => {
    if (b.estimatedDurationMinutes) return b.estimatedDurationMinutes;
    ...
    const ids =
      b.serviceIds && b.serviceIds.length > 0 ? b.serviceIds : b.serviceId ? [b.serviceId] : [];
    ...
    for (const state of vehicleStates) {
      ...
      for (let i = 0; i < unassigned.length; i++) {
        ...
        if (state.vehicle.serviceTypes && state.vehicle.serviceTypes.length > 0) {
          const requiredServiceIds: string[] = [];
          ...
          if (requiredServiceIds.length === 0) continue; // No service info, assume incompatible
      ...
      if (nearestIdx !== -1 && nearestCoords) {
        ...
        const travelTime = state.hasLocation
          ? estimateTravelTimeMinutes(
              state.currentLat,
              state.currentLon,
              nearestCoords.latitude,
              nearestCoords.longitude,
              params.avgTravelSpeedKmph,
              params.trafficBufferMultiplier
            )
          : 0;
...
> {
  ...
  const endIdx =
    options.destinationLocation || options.returnToStart ? bookings.length : bookings.length - 1;
  ...
  if (referenceBookingForEnd?.scheduledStartTime) {
    ...
    plannedEndTime = secondsToTime(
      lastStartSeconds + serviceDurationSeconds + travelFromLastSeconds
    );
 ...
 */
export async function previewRoutePlan(input: PlanRoutesInput): Promise<Result<RoutePlanPreview>> {
  ...
  // Parse date
  const routeDate =
    typeof input.routeDate === 'string' ? new Date(input.routeDate) : input.routeDate;
  ...
  } = await createTimeAwareAllocation(compatibleBookings, vehicles, locationCoordsMap, servicesMap);
 ...
 */
export async function planRoutes(input: PlanRoutesInput): Promise<Result<PlanRoutesResponse>> {
  ...
  if (!allocations || allocations.length === 0) {
    ...
    const result = await createTimeAwareAllocation(
      compatibleBookings,
      vehicles,
      locationCoordsMap,
      servicesMap
  ...
  for (const allocation of allocations) {
    ...
    let bookingsForVehicle: Booking[];
    if (allocation.bookingIds && allocation.bookingIds.length > 0) {
      ...
      bookingsForVehicle = orderedBookings.slice(
        bookingOffset,
    ...
    for (const batch of batches) {
      ...
      if (!optimizeResult.success) {
        warnings.push(`Failed to optimize route for batch: ${optimizeResult.error?.message}`);
      ...
      for (let i = 0; i < orderedBookings.length; i++) {
        ...
        if (booking.serviceItems && booking.serviceItems.length > 0) {
          serviceDurationMinutes = booking.serviceItems.reduce(
        ...
        currentRouteTime = new Date(
          currentRouteTime.getTime() + serviceDurationMinutes * 60 * 1000
        );
      ...
      const routeInput: CreateRouteInput = {
        ...
        plannedEndTime:
          ...
          (() => {
            ...
            const totalSeconds =
              startSeconds + (finalTotalTravelMinutes + calculatedTotalServiceMinutes) * 60;
      ...
      if (!createRouteResult.success) {
        logger.error('Failed to create route', {
          error: JSON.stringify(
            createRouteResult.error,
            Object.getOwnPropertyNames(createRouteResult.error)
          ),
        });
